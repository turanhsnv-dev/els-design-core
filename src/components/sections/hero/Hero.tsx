...
import DarkVeil from "@/components/shared/backgrounds/DarkVeil"; // Yeni komponenti import edirik
    ...
    <section ref={containerRef} className="relative min-h-screen flex items-center pt-24 pb-12 lg:pt-20 overflow-hidden bg-[#0B0C15]">
      ...
      <div className="absolute inset-0 w-full h-full z-0 pointer-events-none">
        <DarkVeil 
            ...
            hueShift={1}
        />
        {/* Overlay - Mətnin oxunması üçün yüngül qaranlıq qat */}
        <div className="absolute inset-0 bg-background-dark/60 backdrop-blur-[1px]"></div>
        
        {/* Dekorativ Spotlar (Optional - DarkVeil ilə birləşəndə dərinlik qatır) */}
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-primary/20 rounded-full blur-[120px] mix-blend-screen opacity-50" />
      </div>

      {/* --- CONTENT (Mətnlər və Floating UI) --- */}
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
        ...
        <div className="flex flex-col gap-6 lg:gap-8 text-center lg:text-left order-2 lg:order-1">
          ...
          {/* Başlık */}
          <h1 className="hero-text text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tight leading-[1.1] text-white">
          ...
          {/* Açıklama */}
          <p className="hero-text text-slate-400 text-base md:text-lg lg:text-xl leading-relaxed max-w-lg mx-auto lg:mx-0 font-light">
        ...
        {/* SAĞ TARAF: YÜZEN UI (SİZİN İSTƏDİYİNİZ KİMİ QALDI) */}
        <div className="relative h-[350px] md:h-[500px] w-full flex items-center justify-center lg:justify-end perspective-1000 order-1 lg:order-2 mb-8 lg:mb-0">
          <div className="relative w-[280px] md:w-[380px] h-full flex items-center justify-center transform scale-90 md:scale-100 origin-center">
            ...
            {/* 1. ARKA KATMAN: Kod Bloğu */}
            <div className="floating-ui absolute -right-4 md:-right-12 top-10 md:top-20 w-56 md:w-64 h-32 md:h-40 bg-[#1e1e2e] rounded-xl border border-white/10 p-4 opacity-40 blur-[1px] transform rotate-12 z-0 hidden sm:block">

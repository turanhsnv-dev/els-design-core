...
export default function Toolkit() {
  return (
    <section
      id="toolkit"
      className="relative w-full flex flex-col items-center py-14 md:py-24 lg:py-32 overflow-hidden bg-white dark:bg-background-dark"
    >
      <RevealContainer className="relative z-10 text-center px-4 mb-10 md:mb-16 lg:mb-20">
        ...
        <RevealItem className="text-4xl md:text-6xl lg:text-7xl xl:text-8xl font-black text-slate-900 dark:text-white mb-4 md:mb-5 tracking-tight leading-none">
          Creative <br />
...
function ToolCard({ tool }: { tool: Tool }) {
  ...
  return (
    <div className="group relative h-full p-6 rounded-2xl border border-black/[0.06] dark:border-white/[0.06] bg-gray-50 dark:bg-[#0e0e13] backdrop-blur-xl cursor-pointer overflow-hidden transition-all duration-700 hover:border-black/[0.14] dark:border-white/[0.14]">
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-5">
          <div className="w-11 h-11 rounded-xl bg-black/[0.04] dark:bg-white/[0.04] border border-black/[0.07] dark:border-white/[0.07] flex items-center justify-center text-slate-600 dark:text-slate-400 group-hover:text-primary group-hover:border-primary/30 transition-all duration-500">
            <Icon size={24} />
          </div>
          <span className="px-2.5 py-1 rounded-full text-[10px] font-medium uppercase tracking-wider border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.02] dark:bg-white/[0.02] text-slate-500">
            {tool.category}
          </span>
        </div>
        <div className="mb-3">
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">{tool.label}</h3>
          <p className="text-[11px] text-primary/70 font-medium tracking-wide">{tool.stats}</p>
        </div>
        <p className="text-slate-500 text-xs leading-relaxed transition-colors duration-300 group-hover:text-slate-600 dark:text-slate-400">
          {tool.description}
        </p>
      </div>
    </div>

...
export default function Testimonials() {
  ...
  return (
    <section
      id="testimonials"
      className="relative w-full py-14 md:py-24 overflow-hidden bg-white dark:bg-background-dark"
    >
      <Container>
...
function ReviewCard({ review, isLarge = false }: { review: Review; isLarge?: boolean }) {
  return (
    <div
      ...
      } rounded-2xl bg-gray-50 dark:bg-[#0e0e13] border border-black/6 dark:border-white/6 hover:border-black/12 dark:border-white/12 transition-all duration-700 cursor-default relative overflow-hidden group`}
    >
      <Quote
        size={isLarge ? 52 : 38}
        className="text-black/4 dark:text-white/4 absolute top-6 left-6 group-hover:text-primary/8 transition-colors duration-700"
      />
      <div className="relative z-10 flex flex-col h-full justify-between pt-10">
        ...
        <div className="flex items-center gap-4">
          <div
            ...
            } rounded-full overflow-hidden border border-black/10 dark:border-white/10 group-hover:border-black/18 dark:border-white/18 transition-colors duration-500 shrink-0`}
          >
            <Image src={review.avatar} alt={review.name} fill className="object-cover" />
          </div>
          <div>
            <h4 className={`text-slate-900 dark:text-white font-medium ${isLarge ? "text-base" : "text-sm"}`}>
              {review.name}
            </h4>
            <p

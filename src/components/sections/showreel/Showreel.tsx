  ...
  return (
    <Container as="section" id="showreel" className="py-14 md:py-24 flex flex-col items-center">
      <RevealContainer className="w-full flex flex-col items-center" stagger={0.12}>
        ...
        <RevealItem className="relative w-full aspect-4/3 md:aspect-21/9 rounded-[28px] md:rounded-[40px] overflow-hidden border border-black/[0.07] dark:border-white/[0.07] shadow-[0_40px_100px_-30px_rgba(0,0,0,0.9)] group">
          ...
          <div className="absolute bottom-0 left-0 w-full p-7 md:p-10 flex items-end justify-between z-20">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-black/60 dark:text-white/60 text-[10px] font-medium uppercase tracking-[0.25em]">
                  Live Process
                </span>
              </div>
              <h4 className="text-slate-900 dark:text-white font-semibold text-xl md:text-2xl max-w-md leading-tight">
          ...
          {!isPlaying && (
            <div className="absolute inset-0 flex items-center justify-center bg-white/40 dark:bg-black/40 backdrop-blur-[2px] transition-all">
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.96 }}
                transition={{ duration: 0.3 }}
                onClick={togglePlay}
                className="w-20 h-20 rounded-full bg-black/8 dark:bg-white/8 backdrop-blur-xl border border-black/15 dark:border-white/15 flex items-center justify-center text-slate-900 dark:text-white hover:bg-black/[0.14] dark:bg-white/[0.14] transition-all duration-300"
              >
...
function IconButton({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="w-11 h-11 rounded-full bg-black/8 dark:bg-white/8 backdrop-blur-md border border-black/12 dark:border-white/12 flex items-center justify-center text-black/70 dark:text-white/70 hover:text-slate-900 dark:text-white hover:bg-black/[0.14] dark:bg-white/[0.14] transition-all duration-300 active:scale-95"
    >
      {children}
    </button>
  );
